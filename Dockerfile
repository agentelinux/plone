...
RUN useradd -m plone; su plone; cd ~/Desktop; wget https://launchpad.net/plone/5.0/5.0rc1/+download/Plone-5.0rc1-UnifiedInstaller.tgz; tar -zxvf Plone-5.0rc1-UnifiedInstaller.tgz; mv Plone-5.0rc1-UnifiedInstaller plone5; cd plone5; sudo ./install.sh standalone; 

EXPOSE 8080

#CMD /bin/bash
CMD ["sudo", "-u", "plone_daemon", "/opt/plone/zinstance/bin/plonectl", "start"]